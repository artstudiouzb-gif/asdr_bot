...
**Рекомендация: публиковать от аккаунта (MTProto)** — это убирает скачивание/заливку
медиа, лимит 50 МБ и половину точек отказа. Bot API оставляем как второй режим,
переключаемый на уровне каждого назначения (`destinations.publish_as`).
...
* если Phase 0 покажет, что MadelineProto на вашем хостинге не работает — включаем план
  Б (C или D) **без изменения панели, БД и правил очистки**: модуль чтения изолирован
  интерфейсом `SourceReader`.
...
1. Хостинг: провайдер, версия PHP (веб и CLI), есть ли SSH, есть ли cron с интервалом 1 минута.
   Идеально — запустить `bin/preflight.php` и прислать вывод (скрипт сделаю первым).
2. Как публикуем: от аккаунта (рекомендую) или от бота.
3. MySQL: доступ или возможность создать базу.
4. Отдельный номер телефона для Telegram-аккаунта-читателя (не личный).
5. Список источников и целевых каналов, где вы уже админ.
6. Домен/поддомен для панели (для HTTPS и webhook).
